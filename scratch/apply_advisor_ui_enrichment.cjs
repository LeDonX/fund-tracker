...
// 1. Insert fundTradingCycle and dcaBargainData useMemos right after usAdvisorData definition
...
const usAdvisorDataMemoReplacement = `    return { f_nasdaq, f_sp500, has_macro_data, spread, activeRule, signal, action, label, color, bg, border, indicator, cardGradient };
  ...
  const fundTradingCycle = useMemo(() => {
    ...
    const todayStr = \`\${year}/\${month}/\subAdvisorData === 'china' ? (\` === 'us' ? (\` \${day}\`;
  ...
  const dcaBargainData = useMemo(() => {
    ...
    if (score >= 75) {
      ...
      desc = isChina
        ...
        : "纳指期指及美股盘前承压。下午 15:00 前申购可锁定今晚暴跌底位净值，是大幅摊薄持仓均价的黄金加仓点！";
    } else if (score >= 55) {
      statusLabel = "🟢 折价温和吸筹";
  ...
  }, [advisorSubTab, chinaAdvisorData, usNasdaqInput, usSp505Input]);`.replace("`\${year}/\${month}/\subAdvisorData === 'china' ? (\` === 'us' ? (\` \${day}\`", "`${year}/${month}/${day}`");

// 2. Insert the two widgets below Weather Station in renderNoviceView
const noviceViewLeftTarget = `            )}
          </div>


        </div>`;

const noviceViewLeftReplacement = `            )}
          </div>

          {/* Card 1: T+1 Fund Trading Cycle timeline info */}
          <div className="bg-white border border-slate-200/60 rounded-3xl p-5 shadow-2xs flex flex-col gap-4 relative overflow-hidden text-left">
            ...
            <div className="flex flex-col gap-3 relative pl-3.5 before:content-[''] before:absolute before:left-1 before:top-1.5 before:bottom-1.5 before:w-0.5 before:bg-slate-100">
              ...
              {/* Step 2: Confirmation */}
              <div className="flex flex-col gap-0.5 text-left relative">
                <span className="absolute -left-4 top-1 w-2.5 h-2.5 rounded-full bg-slate-350 border border-white shadow-3xs flex items-center justify-center"></span>
                <span className="text-xs font-black text-slate-650 leading-none">基金份额确认及可查收益</span>
                <span className="text-[10px] text-slate-450 font-bold leading-normal mt-0.5">份额确认交割日：<span className="text-slate-700 font-extrabold">{fundTradingCycle.confirmationDateStr} (T+1)</span></span>
              </div>

              {/* Step 3: Settle NAV display */}
              <div className="flex flex-col gap-0.5 text-left relative">
                <span className="absolute -left-4 top-1 w-2.5 h-2.5 rounded-full bg-slate-350 border border-white shadow-3xs flex items-center justify-center"></span>
                <span className="text-xs font-black text-slate-650 leading-none">首次净值账面更新与持仓收益查询</span>
                <span className="text-[10px] text-slate-450 font-bold leading-normal mt-0.5">界面持仓更新时间：<span className="text-slate-700 font-extrabold">{fundTradingCycle.navDisplayDateStr} 晚</span></span>
              </div>
          ...
          <div className="bg-white border border-slate-200/60 rounded-3xl p-5 shadow-2xs flex flex-col gap-3 relative overflow-hidden text-left">
            ...
            <div className="flex items-center justify-between gap-4 mt-1 bg-slate-50/60 p-3 rounded-2xl border border-slate-200/30">
              <div className="flex flex-col text-left shrink-0">
                <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">定投吸筹性价比指数</span>
